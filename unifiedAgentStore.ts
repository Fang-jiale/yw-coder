...
import { AIProviderConfig, parseMessageContent, removeSpecialTags, StreamEventItem } from '../../shared/types';
...
interface UnifiedAgentActions {
  ...
  // 流式事件项管理（用于事件流驱动渲染）
  addStreamItem: (item: StreamEventItem) => void;
  updateStreamItem: (id: string, updates: Partial<StreamEventItem>) => void;
    ...
    subscribeWithSelector(
      immer((set, get) => ({
        ...
        appendStreamingMessage: (content) => {
          set((state) => {
            // 只做纯文本聚合，不再解析 think/todo/question 结构
            // 结构解析现在由主进程的 streamEvent 事件处理
        ...
        updateStreamItem: (id, updates) => {
          set((state) => {
            const index = state.streamingItems.findIndex((item) => item.id === id);
            if (index !== -1) {
              Object.assign(state.streamingItems[index], updates);
            }
          });
        },
...
export function setupUnifiedAgentEventListeners(): void {
  ...
  if (isEventListenersSetup) {
    console.log('[UnifiedAgentStore] Event listeners already setup, skipping');
    return;
  }
  isEventListenersSetup = true;

  console.log('[UnifiedAgentStore] Setting up event listeners...');
  console.log('[UnifiedAgentStore] electronAPI available:', !!window.electronAPI);
  console.log('[UnifiedAgentStore] unifiedAgent available:', !!window.electronAPI?.unifiedAgent);
  console.log('[UnifiedAgentStore] onStatusChange available:', !!window.electronAPI?.unifiedAgent?.onStatusChange);
  ...
  window.electronAPI?.unifiedAgent?.onStepStart?.((data: any) => {
    console.log('[UnifiedAgentStore] onStepStart:', data.taskId);
